...
function draw() {
    ...
    let startcols = Math.max(
        0, Math.floor(cameraX / Theme.sizes.tile)
    );
    let endcols = Math.min(
        cols, startcols + Math.ceil(canvas.width / Theme.sizes.tile) + 1
    );

    let startrows = Math.max(
        0, Math.floor(cameraY / Theme.sizes.tile)
    );
    let endrows = Math.min(
        rows, startrows + Math.ceil(canvas.height / Theme.sizes.tile) + 1
    );
    ...
    for (let i = 0; i < entities.length; i++) {
        ...
        // Let the new sprite.js file handle the complicated drawing!
        if (typeof drawEntity === "function") {
            drawEntity(ctx, crnEnt, cameraX, cameraY);
        }
    }

    // Increment global animation frame once per render tick
